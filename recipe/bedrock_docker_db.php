...
task( 'pull:db', function () use ( $getLocalEnv, $getRemoteEnv, $urlToDomain ) {
	...
	runLocally( "docker-compose run --rm ".get( 'wp_container' )." wp db export {$backupFilename} --allow-root" );

	// Empty local DB
	writeln( "<comment>Reset server DB</comment>" );
	runLocally( "docker-compose run --rm ".get( 'wp_container' )." wp db reset --allow-root" );

	// Import export file
	// todo: change path of the downloaded file
	writeln( "<comment>Importing {$downloadedExport}</comment>" );
	runLocally( "docker-compose run --rm ".get( 'wp_container' )." wp db import ../{$exportFilename} --allow-root" );
	...
	runLocally( "docker-compose run --rm ".get( 'wp_container' )." wp search-replace '{$remoteUrl}' '{$localUrl}' --skip-themes --url='{$remoteDomain}' --network --allow-root" );
	// Also replace domain (multisite WP also uses domains without protocol in DB)
	runLocally( "docker-compose run --rm ".get( 'wp_container' )." wp search-replace '{$remoteDomain}' '{$localDomain}' --skip-themes --url='{$remoteDomain}' --network --allow-root" );
...
task( 'push:db', function () use ( $getLocalEnv, $getRemoteEnv, $urlToDomain ) {
	...
	runLocally( "docker-compose run --rm ".get( 'wp_container' )." wp db export {$exportFilename} --allow-root");
